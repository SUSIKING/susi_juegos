...
export class LaberinOjoGame {
  ...
  newLevel(level){
    ...
    this.player = {
      x:start.x,y:start.y,r:this.cell*.32,
      dir:{x:0,y:0},wanted:{x:0,y:0},speed:0,
      lives:5,damageUntil:0,dead:false,penalty:0,blink:0,teleportUntil:0,safe:[]
    };
    this.pushSafePosition();
    this.input = { active:false, id:null, sx:0, sy:0, x:0, y:0, power:0, dir:{x:0,y:0} };
  ...
  isWallAt(px, py){
    ...
    return x < 0 || y < 0 || x >= this.cols || y >= this.rows || this.maze[y][x] === WALL;
  }

  circleHitsWall(x, y, r){
    const pts = [[0,0],[r,0],[-r,0],[0,r],[0,-r],[r*.7,r*.7],[-r*.7,r*.7],[r*.7,-r*.7],[-r*.7,-r*.7]];
    return pts.some(([dx, dy]) => this.isWallAt(x + dx, y + dy));
  }

  pushSafePosition(){
    if(!this.player || this.circleHitsWall(this.player.x, this.player.y, this.player.r)) return;
    this.player.safe.push({ x:this.player.x, y:this.player.y });
    if(this.player.safe.length > 18) this.player.safe.shift();
  }
  ...
  rollbackPlayer(){
    ...
    if(target && !this.circleHitsWall(target.x, target.y, this.player.r)){
      this.player.x = target.x;
      this.player.y = target.y;
      this.player.safe = [target];
      return;
    }
    ...
    if(fallback){
      ...
      this.player.safe = [fallback];
    }
  }

  nearbyHealthyCenters(maxDist){
    const here = this.pixelToCell(this.player.x, this.player.y);
    const radiusCells = Math.ceil(maxDist / this.cell) + 1;
    const out = [];

    for(let gy = here.y - radiusCells; gy <= here.y + radiusCells; gy++){
      for(let gx = here.x - radiusCells; gx <= here.x + radiusCells; gx++){
        if(gx < 0 || gy < 0 || gx >= this.cols || gy >= this.rows || this.maze[gy][gx] !== PATH) continue;
        const c = this.cellCenter({ x:gx, y:gy });
  ...
  settleAfterRelocation(){
    this.player.dir = { x:0, y:0 };
    this.player.speed = 0;
    if(this.canMove(this.player.wanted)) this.player.dir = { ...this.player.wanted };
    this.pushSafePosition();
  }
  ...
  teleportPlayer(){
    ...
    if(centers.length){
      const pick = centers[Math.min(centers.length - 1, Math.floor(Math.random() * Math.min(8, centers.length)))];
      this.player.x = pick.x;
      this.player.y = pick.y;
      this.player.teleportUntil = performance.now() + 280;
      this.settleAfterRelocation();
    ...
    this.rollbackPlayer();
    this.settleAfterRelocation();
    this.player.teleportUntil = performance.now() + 180;
    this.audio.beep(180, .05, 'sawtooth', .035);
  }
  ...
  setPointer(e){
    ...
    this.input.power = clamp(Math.hypot(dx, dy) / 80, 0, 1);
    this.input.dir = this.dominant(dx, dy);
    this.player.wanted = this.input.dir;
  }
  ...
  canMove(dir){
    ...
    const nx = this.player.x + dir.x * this.cell * .42;
    const ny = this.player.y + dir.y * this.cell * .42;
    return !this.circleHitsWall(nx, ny, this.player.r * .9);
  }

  nearCellCenter(){
    const cx = this.ox + (Math.floor((this.player.x - this.ox) / this.cell) + .5) * this.cell;
    const cy = this.oy + (Math.floor((this.player.y - this.oy) / this.cell) + .5) * this.cell;
    return {
      ok:Math.abs(this.player.x - cx) < Math.max(2, this.cell * .13) && Math.abs(this.player.y - cy) < Math.max(2, this.cell * .13),
      cx, cy
    };
  }
  ...
  update(dt, now){
    ...
    if(center.ok){
      this.player.x = center.cx;
      this.player.y = center.cy;
      if(!this.circleHitsWall(this.player.x, this.player.y, this.player.r)) this.pushSafePosition();
      if(this.canMove(this.player.wanted)) this.player.dir = { ...this.player.wanted };
      else if(!this.canMove(this.player.dir)) this.player.dir = { x:0, y:0 };
    } else if(!this.player.dir.x && !this.player.dir.y && this.canMove(this.player.wanted)) {
      this.player.dir = { ...this.player.wanted };
    }

    const pow = this.input.active ? this.input.power : Math.max(0, this.player.speed / this.levelParams.speedMax - dt * 4);
    const shaped = Math.pow(pow, 1.35);
    this.player.speed = pow > 0 ? this.levelParams.speedMin + (this.levelParams.speedMax - this.levelParams.speedMin) * shaped : 0;

    const nx = this.player.x + this.player.dir.x * this.player.speed * dt;
    const ny = this.player.y + this.player.dir.y * this.player.speed * dt;

    if(!this.circleHitsWall(nx, ny, this.player.r)){
      this.player.x = nx;
      this.player.y = ny;
      this.pushSafePosition();
    } else {
    ...
    if(Math.hypot(this.player.x - goal.x, this.player.y - goal.y) < this.cell * .45){
  ...
  draw(){
    ...
    this.drawGoal();
    this.drawPlayer();
    if(this.input?.active) this.drawJoystick();
  }
  ...
  drawJoystick(){
    ...
    ctx.beginPath();
    ctx.arc(this.input.sx + clamp(dx, -80, 80), this.input.sy + clamp(dy, -80, 80), 18, 0, Math.PI * 2);
    ctx.fill();
    ctx.restore();
  }
